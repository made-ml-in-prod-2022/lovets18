...
def validation(
    model_dir_from: str, data_dir_from: str, metric_dir: str
) -> None:
    ...
    targets = pd.read_csv(
        os.path.join(data_dir_from, VAL_TARGETS_FILENAME)
    ).to_numpy()
    ...
    with open(os.path.join(metric_dir, METRICS_FILENAME), "w") as file:

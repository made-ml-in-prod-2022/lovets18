...
def generate(dir_in: str) -> None:
    ...
    features, targets = make_classification(n_samples=N_ROWS + np.random.randint(100) - 50)
    features = pd.DataFrame(features, columns=[f'col_{i}' for i in range(features.shape[1])])

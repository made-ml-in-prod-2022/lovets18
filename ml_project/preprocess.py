...
class Standartizer(BaseEstimator, TransformerMixin):
    ...
    @staticmethod
    def standartize(
            data: pd.DataFrame, save_dir: str, test_mode=False
    ) -> pd.DataFrame:
...
def preprocess_data(
        data: pd.DataFrame, drop_low_cor: list
) -> pd.DataFrame:
    """Fillna and drop columns if choosed"""
    data = data.fillna(0)
    if len(drop_low_cor) > 0:
        data = data.drop(drop_low_cor, axis=1)
        logging.debug('features %s were dropped', ', '.join(drop_low_cor))
    return data
